...
interface Column {
  ...
  field: string;
  show?: boolean;
  type?: 'button' | 'btn-toggle' | 'img' | 'binario' | 'button-delete' | 'btn-img-popup'; // <-- agregamos 'btn-img-popup'
  trueValue?: string;
  falseValue?: string;
  sumable?: boolean;
  totalLabel?: string;
  ...
  // Propiedades específicas para 'btn-img-popup':
  btnLabel?: string;         // Texto que aparecerá en el botón
  imgDescripcion?: string;   // Descripción o alt text de la imagen
}

@Component({
  selector: 'app-general-table-reportes',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './general-table-reportes.component.html',
  styleUrls: ['./general-table-reportes.component.scss'],
  animations: [
...
export class GeneralTableReportesComponent implements OnInit , OnChanges {
  // Entradas básicas
  @Input() columns: any[] = [];
  @Input() data: any[] = [];
...
ngOnInit(): void {
   // console.log('🟡 ngOnInit - filtros recibidos:', this.filters);
    ...
    if (f.type.startsWith('date')) {
      ...
      //console.log(`🟨 Filtro "${f.field}" - dateDefault antes:`, defaultDate);
  ...
  updatePaginatedData(): void {
   ...
   // this.calculateTotals(filteredData);
    ...
    const pageSlice = filteredData.slice(startIndex, endIndex).map(row => {
    ...
    this.paginatedData = pageSlice;

    // Recalculamos totales, PERO sólo sobre la página actual
    this.calculateTotals(this.paginatedData);
  ...
  onDateFilterChange(field: string, type: 'start'|'end', isoStr: string) {
    ...
    public modalImageDescripcion: string = '';

    // Método para abrir el modal con la URL y descripción
